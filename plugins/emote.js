...
module.exports = function (client) {
	...
	var emotes = {
		...
		backpat: [
			[20, 'gives {0} a pat on the back.', 'pats on the back'],
			[5, 'rubs {0}\' on the back, causing {0} to burst into treats', 'hardcore pats on the back'],
		],
		
		karatechop: [
			[20, 'karate chops {0} for'+(Math.floor(Math.random()*100)+1)+' damage!', 'karatechops'],
			[5, 'karate chops {0} in half! {0} takes '+(Math.floor(Math.random()*901)+100)+' damage!!!', 'killer karatechops'],
		],
	};
